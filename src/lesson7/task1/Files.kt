...
fun deleteMarked(inputName: String, outputName: String) {
    val buff = File(outputName).bufferedWriter()
    for (line in File(inputName).readLines()) {
...
fun centerFile(inputName: String, outputName: String) {
    ...
    val text = file.map { it.trim() }
    var lengthOfMaxLine = 0
    for (line in text) if (line.length > lengthOfMaxLine) lengthOfMaxLine = line.length
    val buff = File(outputName).bufferedWriter()
    for (line in text) {
        for (i in 1..(lengthOfMaxLine - line.length) / 2) buff.write(" ")
        buff.write(line)
        buff.newLine()
    }
    buff.close()
...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    val listOfCorrectedLines = mutableListOf<String>()
    var maxLengthLine = 0

    for (line in reader) {
        var spaceCleaner = line.trim()
        while (spaceCleaner.contains("  "))
            spaceCleaner = spaceCleaner.replace("  ", " ")
